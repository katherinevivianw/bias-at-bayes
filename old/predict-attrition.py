...
X_VALUES = ["Female", "OverTime", "JobSatisfaction", "MonthlyIncome"]
...
def joint_prob(x_row, y, all_p_x_given_y, p_y):
    """
    ...
    for x_val in x_row.keys():
        if x_val != Y_VAL:
            if x_row[x_val] == 1:      # curr x_val is 1 in the given x vector
                prob_y_given_x *= all_p_x_given_y[x_val][y]
            else:
                prob_y_given_x *= 1 - all_p_x_given_y[x_val][y]
...
def get_user_inputs():
    ...
    user inputs.
    """
    user_row = dict.fromkeys(X_VALUES, -1)

    user_row["Female"] = int(input("Enter 1 or 0 for female. "))
    user_row["OverTime"] = int(input("Enter 1 or 0 for overtime. "))
    user_row["JobSatisfaction"] = int(input("Enter 1 or 0 for job satisfaction. "))
    user_row["MonthlyIncome"] = int(input("Enter 1 or 0 for monthly income. "))

    return user_row

def predict_attrition(all_p_x_given_y, p_y, data):
    """
    Predicts the probability of attrition given the user's input x vlaues.
    """
    # get user inputs for x values 
    user_row = get_user_inputs()
...
def main():
    ...
    all_p_x_given_y = get_all_p_x_given_y(Y_VAL, training)
    #print(f"P(X_i | Y) =  {all_p_x_given_y}")
    p_y = get_p_y(Y_VAL, training)
    #print(f"P(Y) =  {p_y}")

    # get attrition prediction
    print(f"P(Attrition) =  {predict_attrition(all_p_x_given_y, p_y, training)}")
